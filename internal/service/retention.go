...
func (s *RetentionService) ArchiveQCEvents(before time.Time) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("archive qc_events: begin tx: %w", err)
	}
...
WHERE status = ? AND detected_at < ?`, "resolved", before.UTC())
	...
	res, err = tx.ExecContext(ctx, `
DELETE FROM qc_events WHERE status = ? AND detected_at < ?`, "resolved", before.UTC())
	if err != nil {
	...
	if deleted != moved {
		return 0, fmt.Errorf("archive qc_events: copied %d rows but deleted %d; rolling back", moved, deleted)
	}
	if err := tx.Commit(); err != nil {
		return 0, fmt.Errorf("archive qc_events: commit: %w", err)
	}
	return moved, nil
}
